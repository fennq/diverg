...
- **Investigations API**: `POST /api/investigation/blockchain` is a fast RPC snapshot (Helius for Solana; EVM balance/nonce plus optional recent mainnet txs when `ETHERSCAN_API_KEY` is set on the server). `POST /api/investigation/blockchain-full` runs the full `blockchain_investigation` skill (crime-oriented report, flow graph, explorer links; server API keys only; can take ~1–2 minutes). Dashboard Investigation page includes both; deep link ` /dashboard/?page=investigation&inv_address=…&inv_chain=solana|ethereum|…`. Chrome extension: address harvester in the popup → Dashboard or **Full API** (paste dashboard JWT in Extension Options; set `DIVERG_ALLOW_EXTENSION_CORS=1` on the API host so the extension origin is allowed for CORS).
